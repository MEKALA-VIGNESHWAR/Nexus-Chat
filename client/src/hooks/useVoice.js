/**
 * Voice Room WebRTC Custom Hook
 * Wires multi-user peer-to-peer mesh topology voice communication.
 * Handles microphone permissions, local media streams, peer connections,
 * ICE candidates exchange, signaling, mute states, and active speaker visual updates.
 */
...
export function useVoice(roomId) {
  ...
  const [peers, setPeers] = useState({}); // { [userId]: { socketId, username, avatar, stream, isMuted } }
  ...
  const peersRef = useRef({}); // Map of RTCPeerConnection instances keyed by userId
  const peerStatesRef = useRef({}); // Map of peer UI states keyed by userId
   ...
   */
  const createPeerConnection = useCallback((peerUserId, peerSocketId, stream) => {
  ...
  const joinVoiceRoom = useCallback(async () => {
    ...
    try {
      ...
      emit(SOCKET_EVENTS.VOICE_JOIN, { roomId }, async (response) => {
        if (!response.success) {
  ...
  const toggleMute = useCallback(() => {
    if (localStreamRef.current) {
      ...
      if (audioTrack) {
        ...
        // Broadcast mute state update to signaling server if needed
        // (Simplified here; WebRTC track disabled does not need renegotiation)
      }
    }
  }, []);
